...
func hordeState(status string, withInstance bool) *fabricastate.State {
	...
	st.UpsertModule("horde", "ami-0abc123def456789", status, resources)
	return st
}
...
func TestDestroyHappyPathModuleRemovedFromState(t *testing.T) {
	...
	if m := lastState.GetModule("horde"); m != nil {
		t.Error("horde module must be removed from state after successful destroy")
	}
}
...
func TestDestroySGOnlyNoInstance(t *testing.T) {
	var out bytes.Buffer
	st := hordeState("provisioning", false) // SG only
	var deletedTypes []string
	c := newTestCommand(&out, st, nil)
	c.assumeYes = true
	c.deleteResource = func(_ context.Context, r *cloud.Resource) error {
...
func TestDestroyConfirmationPhrase(t *testing.T) {
	...
	if capturedPhrase != "destroy horde 123456789012" {
		t.Errorf("phrase = %q, want %q", capturedPhrase, "destroy horde 123456789012")
	}
	assertContains(t, out.String(), "destroy horde 123456789012")
}
...
func findResource(m *fabricastate.ModuleState, typeName string) (fabricastate.ModuleResource, bool) {
	for _, r := range m.Resources {
