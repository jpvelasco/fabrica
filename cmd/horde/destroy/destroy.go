...
// New returns the "horde destroy" subcommand. It accepts RuntimeSource and
...
func (c command) applyDestroy(ctx context.Context, st *fabricastate.State, m *fabricastate.ModuleState, resources []cloud.Resource) error {
	...
	fmt.Fprintln(c.out)
	fmt.Fprintln(c.out, "Unreal Horde build coordinator destroyed.")
...
func confirmPhrase(account string) string {
	return fmt.Sprintf("destroy horde %s", account)
}
...
func (c command) printNotProvisioned() {
	...
	fmt.Fprintln(c.out, "Horde is not provisioned. Nothing to destroy.")
}
...
func (c command) printDryRun(m *fabricastate.ModuleState, resources []cloud.Resource) {
	...
	fmt.Fprintln(c.out, "Unreal Horde build coordinator (destroy dry run)")
...
func (c command) printDestroyPlan(m *fabricastate.ModuleState, resources []cloud.Resource) {
	fmt.Fprintln(c.out, "Unreal Horde build coordinator — destroy plan")
	fmt.Fprintln(c.out, strings.Repeat("-", lineWidth))
	fmt.Fprintf(c.out, "  AMI ID:   %s\n", m.Version)
	fmt.Fprintf(c.out, "  Status:   %s\n", m.Status)
	fmt.Fprintln(c.out)
	...
	fmt.Fprintln(c.out)
	fmt.Fprintln(c.out, "IRREVERSIBLE: This will permanently delete the Horde coordinator and its data.")
}
