...
func TestNewWiresSubcommands(t *testing.T) {
	...
	if cmd.Use != "workstation" {
		t.Fatalf("Use = %q, want workstation", cmd.Use)
	}
	if cmd.Short != "Manage cloud workstations" {
...
func TestNewHasExpectedSubcommands(t *testing.T) {
	...
	for _, want := range []string{"create", "list", "stop", "start", "terminate", "ami"} {
		if !names[want] {
