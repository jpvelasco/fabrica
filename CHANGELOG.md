...
- **Lore TLS foundation** — `lore.tls` config section (`LoreTLSConfig` with `Enabled`, `CertPath`, `KeyPath`) wired into `fabrica.yaml` and the cloud-init template. Full TLS support (certificate provisioning, ACM, rotation) deferred to V2. Design note at `docs/lore-tls.md`.
