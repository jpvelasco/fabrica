...
- **Workstation create accepted a stock Ubuntu AMI as DCV-ready** — create now rejects Canonical Ubuntu 22.04 server image names and cloud-init fails closed if `dcv` is missing (no boot-time apt/snap install, no `|| true` on DCV units). `fabrica workstation ami build` generates Image Builder artifacts (DCV + SSM). See `docs/workstation-ami.md`. (#420)
