...
- **Horde cold boot: first `docker compose up -d` aborted cloud-init on mongo healthcheck timeout** — on a fresh instance mongo's `mongosh` healthcheck (timeout: 5s) fails for the first ~30s, so with `depends_on: mongodb: condition: service_healthy` the bare `docker compose up -d` in cloud-init (under `set -euo pipefail`) exited non-zero before the stack converged (~90s). The script aborted, the `horde-ready` sentinel was never written, and `horde status` stayed `provisioning` forever. Cloud-init now retries `docker compose up -d` (8 × 20s) before the HTTP readiness probe and still fails closed on exhaustion (clear log line, non-zero exit). No AMI rebake required. (#446)
- **Workstation cloud-init aborted on DCV 2025.0.x — session and password never applied** - the script called the removed `dcv configure-session` / `dcv configure` subcommands first (under `set -euo pipefail`), so on DCV 2025.0.x AMIs the persistent session and the generated DCV password were never applied even though the DCV server ran and answered 200 on 8443. The DCV section now targets the current CLI surface (2023.0+): `dcv set-config --section connectivity --key idle-timeout` for the idle timeout, `dcv create-session --type virtual --user ubuntu --owner ubuntu` for the persistent `workstation` session, and `chpasswd` for the `ubuntu` user password (the credential file already documents a single `dcv_session_password` with no user key). The script fails closed when the `ubuntu` user is missing or the session does not appear in `dcv list-sessions` within 3 minutes, and the AMI runbook (`docs/workstation-ami.md`) documents the `dcv list-sessions` smoke check as the boot verification (HTTPS 200 on 8443 is not proof the session setup ran). The DCV server is also started **before** the session is created: with `dcvserver` stopped, `dcv create-session` exits 0 but the session is never persisted (reproduced and fixed live on a DCV 2025.0.x instance; #438).
- **`cloud.aws.tags` not applied on Cloud Control module creates** — the provider's create path (`Create` / `createAsync`) and the CodeBuild `EnsureProject` SDK path both dropped operator tags: only the built-in `ManagedBy` / `FabricaModule` / `FabricaVersion` stamps landed. `newProvider` now copies `cloud.aws.tags` into the resource clients (`operatorTags`), the Cloud Control create path merges them via `injectFabricaTags`, and `EnsureProject` merges them into the project tags. Operator tags are additive: same-key entries for the reserved identity tags (`ManagedBy` / `FabricaModule` / `FabricaVersion`) are dropped, since teardown/drift/cost key off those values — an operator shadowing them would leak resources on `destroy --all`. `WithRegion` carries them to multi-region (DDC edge) creates. (#417)
...
- **Module create/setup skipped the DynamoDB state lock** — `perforce`/`horde`/`lore`/`workstation create` and `ddc`/`ci`/`deploy setup` (plus `ddc region add`) read-modify-wrote `.fabrica/state.json` without `AcquireStateLock`, so two concurrent creates could clobber state or double-provision. `provision.RunCreate` now takes the account-level lock for every create module; the remaining setup paths acquire the same lock after `--dry-run` and before the first state read. Held locks abort before any state or AWS write. (#365)
- **Lore S3-store destroy skipped DynamoDB tables and dropped them from state** — `fabrica lore destroy` built its plan from `loreResourceOrder` but a filtered type/order path omitted the four `AWS::DynamoDB::Table` rows already present in state (`<bucket>-fragments/-metadata/-mutable/-locks`), deleting only 5 of 9 resources and then removing the `lore` module from state via `teardown.RemoveModule`. The four tables were orphaned in `us-west-2` and a second `lore destroy` was a no-op. The filter now honors every `AWS::DynamoDB::Table` row in state, in documented order (instance → profile → role → tables → bucket → SG), with table-only leftovers still deletable; dry-run and live share the same `loreResourceOrder` builder.
