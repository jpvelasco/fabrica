...
- **Lore TLS foundation** — `lore.tls` config section (`LoreTLSConfig` with `Enabled`, `CertPath`, `KeyPath`) added to `fabrica.yaml`. Config fields are parsed but not yet wired into cloud-init (no-op until V2). Design note at `docs/lore-tls.md`.
