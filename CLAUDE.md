...
- **Deploy torn down with `all=true`** — When `destroy --all` tears down the deploy module via `deploydestroy.NewTeardown(rt, out, spec(true))`, it deletes everything: fleets, builds, alias, and IAM role. This differs from plain `fabrica deploy destroy`, which deletes fleets + builds but retains the alias and role for re-promotion.
