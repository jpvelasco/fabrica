...
| `workstation` | `create`, `list`, `stop`, `start`, `schedule`, `terminate`, `ami build` | Provisions a NICE DCV cloud workstation on EC2 (AMI-first, g4dn.xlarge default); allows TCP 8443 inbound; writes DCV session credentials to `.fabrica/workstation-credentials.yaml`; supports stop/start via EC2InstanceManager and permanent termination; `ami build` generates Image Builder artifacts; create rejects stock Ubuntu AMIs |
...
**Distributed state locking** — every state-mutating flow acquires the account-level DynamoDB lock via `provision.AcquireStateLock(ctx, rt, operation)` at entry and defers the release. `--dry-run` uses `AcquireStateLockUnlessDryRun` and must not write a lock row. Shared create (`provision.RunCreate`) takes the lock for perforce/horde/lore/workstation; setup commands that do not use `RunCreate` (`ci`/`deploy`/`ddc setup`, `ddc region add`) acquire it themselves. Nested orchestration (`destroy --all` → module teardowns) inherits the lock through a ctx sentinel and no-ops. The lock carries a 15-minute TTL with stale takeover; a live holder heartbeats `Renew` at TTL/3 so a long promote cannot be stolen, while a crashed process still expires. Providers without `cloud.StateLockManager` (fakes/E2E) get a silent no-op. `setup` (and any command on an unbootstrapped account) proceeds unlocked when the table is absent — the adapter maps ResourceNotFoundException to `cloud.ErrLockTableMissing`; don't "fix" this into a hard failure, it's the bootstrap ordering.
...
**AWS auth** — SDK default credential chain plus optional `cloud.aws.profile` (IAM Identity Center / SSO named profiles included). Expired SSO sessions are detected from the AWS error text and tell the operator to run `aws sso login`. Do not add a second login command; the AWS CLI owns the browser SSO flow.
...
`.golangci.yml` (v2 schema) starts from `default: none` and explicitly enables: `errcheck`, `govet`, `ineffassign`, `staticcheck`, `unused`, `gocritic`, `misspell`, `unconvert`, `gosec`, `dupl`. `gofmt` is the only formatter. `gosec` excludes G104 (best-effort cleanup), G204/G702/G703 (subprocess/taint-analysis noise for a CLI tool), G301/G306 (standard dir/file perms), G304 (config-file reads via variable path) — match these rationales before adding new suppressions. Codacy mirrors this via `.codacy.yml` (govet + staticcheck engines). The Opengrep finding `Semgrep_go.lang.security.audit.xss.import-text-template.import-text-template` on `cmd/lore/ami/build.go` is dispositioned as a false positive via the Codacy API (`codacy issue <resultDataId> --ignore --ignore-reason FalsePositive`) — every `text/template` use renders local build artifacts or cloud-init, never HTML; re-check if a web surface appears. New occurrences still surface as fresh findings (the pattern stays enabled). Note: inline `// nosemgrep` comments are ignored by the managed scan.
...
- **Lore TLS is AMI-path based** — `lore.tls.enabled` plus absolute `certPath`/`keyPath` are written into cloud-init (`[server] tls_cert`/`tls_key`) and verified on the instance at boot. Fabrica does not provision, rotate, or push certificates. Health probe remains HTTP on 41339. See `docs/lore-tls.md`.
