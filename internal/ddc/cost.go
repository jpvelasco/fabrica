...
func CostResources(cfg config.DDCConfig) []cost.Resource {
	...
	out := []cost.Resource{
		{TypeName: TypeAWSS3Bucket, Name: bucket},
	...
	if backend == BackendScylla {
		...
		out = append(out,
			cost.Resource{TypeName: TypeAWSEC2Instance, Name: scyllaType},
			cost.Resource{TypeName: TypeAWSEC2Volume, Name: fmt.Sprintf("gp3-%dGiB", scyllaVol)},
		)
	}
	return out
}
