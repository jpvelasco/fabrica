...
// Fabrica does not generate Lore secrets; TLS is operator-baked into the AMI.
func FormatLore(grpcPort, httpPort int) string {
	return fmt.Sprintf(`# Lore server connection notes
# Auth is unset. When lore.tls.enabled is true, clients must trust the AMI cert
# or use the client's insecure / skip-verify option.
